...
--
-- Name: validar_una_correcta(); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.validar_una_correcta() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
  cnt INTEGER;
BEGIN
  -- Si la operación es DELETE y la borrada era correcta,
  -- después seguiríamos sin correcta, así que lo bloqueamos
  IF TG_OP = 'DELETE' AND OLD.es_correcta THEN
    RAISE EXCEPTION 'No se puede borrar la respuesta correcta; cada pregunta debe tener una correcta.';
  END IF;

  -- Si INSERT o UPDATE marca como correcta, contamos cuántas ya había
  IF (TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND NEW.es_correcta AND NOT OLD.es_correcta)) THEN
    ...
    IF cnt > 0 THEN
      RAISE EXCEPTION 'La pregunta % ya tiene una respuesta correcta.', NEW.pregunta_id;
    END IF;
  END IF;
...
ALTER FUNCTION public.validar_una_correcta() OWNER TO postgres;
...
--
-- Name: respuestas trg_validar_una_correcta; Type: TRIGGER; Schema: public; Owner: postgres
--

CREATE TRIGGER trg_validar_una_correcta BEFORE INSERT OR DELETE OR UPDATE ON public.respuestas FOR EACH ROW EXECUTE FUNCTION public.validar_una_correcta();
